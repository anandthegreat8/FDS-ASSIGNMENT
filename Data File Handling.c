...
void ins(student *s,int i,int n)						//Insert in between Function
{
	int l=i+1;
...
void output(student *s,int i)							//Output Data															    //output function
{	FILE *fp;
	fp=fopen("D:\\output.txt", "a");
	fprintf(fp,"\n%d\t\t%lld\t%.2f\t\t%s",(s+i)->roll_no,(s+i)->mobile_no,(s+i)->per,(s+i)->name);
	fflush(stdin);
		printf("\n%d\t\t%lld\t%.2f\t\t",(s+i)->roll_no,(s+i)->mobile_no,(s+i)->per);
		printf("%s",(s+i)->name);
		fclose(fp);
}
void append(student *s, int i)							//update Function
{	int m,j=0,b;
	char ch;
	...
	switch(m)
	{
		...
		case 3:
		{
			printf("\nENTER THE SUBJECT NUMBER YOU WANT TO EDIT:");
			scanf("%d",&b);
